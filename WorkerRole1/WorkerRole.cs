        ...
        public override void Run()
        {
            ...
            while (true)
            {
                ...
                if (checkCommandQ != null)
                {
                    ...
                    if (command == "start")
                    {
                        string root_url = "http://www.cnn.com";
                        CloudQueueMessage messagePush = new CloudQueueMessage(root_url);
                ...
                if (go)
                {
                    ...
                    if (linkPop != null)
                    {
                        ...
                        if (isDisallowed(url, cnn_disallow) == false && visitedURLMain.ContainsKey(url) == false)
                        {
                            ...
                            {
                                //Azure table insert
                                AzureTableEntity newURL = new AzureTableEntity("URL Partition");
                                newURL.url = url;
                                newURL.PageTitle = pagetitle;
                                TableOperation insertOperation = TableOperation.Insert(newURL);
                            ...
                            foreach (Match item in matches)
                            {
                                ...
                                {
                                    Trace.TraceInformation("Added: " + link_new);
                                    linkQueue.AddMessage(messagePush);
